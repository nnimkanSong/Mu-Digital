...
export default function SelectionPage() {
    ...
    // State เก็บว่าเลือกเพศไหน (ถ้ามีค่า = กำลังโหลด)
    const [targetGender, setTargetGender] = useState<string | null>(null);

    const selectGender = (gender: string) => {
        if (targetGender) return; // ป้องกันกดซ้ำ
        setTargetGender(gender);  // Trigger ให้ LoadingScreen แสดง
    };

    // ฟังก์ชันนี้จะถูกเรียกโดย LoadingScreen เมื่อโหลดเสร็จ
    const handleFinishLoading = () => {
        router.push(`/birthday?gender=${targetGender}`);
    };
    ...
    return (
        <div className="min-h-screen bg-[#0f0c29] bg-gradient-to-b from-[#0f0c29] via-[#1a1a2e] to-[#0f0c29] flex flex-col items-center justify-center px-4 py-10 relative overflow-hidden font-serif">
            ...
            {/* แสดง LoadingScreen เมื่อมีการเลือกเพศ */}
            {targetGender && (
                <LoadingScreen 
                    duration={1500} // โหลด 1.5 วินาที
            ...
            <div className="z-10 w-full max-w-7xl">
                ...
                <div className="flex flex-col md:flex-row gap-6 sm:gap-12 justify-center items-stretch">
                    ...
                    <button
                        ...
                        disabled={targetGender !== null}
                        className={`group relative flex-1 flex flex-col items-center p-5 sm:p-10 rounded-[2.5rem] border border-white/5 bg-white/[0.02] backdrop-blur-2xl transition-all duration-700 hover:bg-blue-900/10 hover:border-blue-500/40 hover:-translate-y-4 hover:shadow-[0_20px_80px_rgba(59,130,246,0.2)] cursor-pointer ${targetGender ? 'opacity-50 pointer-events-none grayscale' : ''}`}
                    >
                    ...
                    <button
                        ...
                        disabled={targetGender !== null}
                        className={`group relative flex-1 flex flex-col items-center p-5 sm:p-10 rounded-[2.5rem] border border-white/5 bg-white/[0.02] backdrop-blur-2xl transition-all duration-700 hover:bg-pink-900/10 hover:border-pink-500/40 hover:-translate-y-4 hover:shadow-[0_20px_80px_rgba(236,72,153,0.2)] cursor-pointer ${targetGender ? 'opacity-50 pointer-events-none grayscale' : ''}`}
                    >
                        <div className="relative w-full aspect-[3/4] max-h-[50vh] flex items-center justify-center overflow-visible cursor-pointer">
