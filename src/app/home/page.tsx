  ...
  return (
    <div className="relative h-screen w-full overflow-hidden font-serif bg-black">
        ...
        <div className="absolute inset-0 pointer-events-none z-20">
          {stars.map((s, i) => (
            <div key={i} className="absolute bg-white rounded-full animate-pulse"
              style={{ top: s.top, left: s.left, width: s.size, height: s.size, animationDuration: s.duration, opacity: s.opacity }}
            />
          ))}
        </div>
      )}
      ...
      <div className="relative z-30 flex h-full flex-col items-center justify-center text-center px-6 pointer-events-none">
        <motion.p 
          initial={{ opacity: 0, letterSpacing: "0.1em" }} animate={{ opacity: 1, letterSpacing: "0.3em" }} transition={{ duration: 1 }}
        ...
        <motion.div initial={{ width: 0 }} animate={{ width: "128px" }} transition={{ delay: 0.5, duration: 1 }}
