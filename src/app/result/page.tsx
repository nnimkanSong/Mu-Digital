            ...
            <div className="z-10 pb-8 flex flex-col items-center min-h-[140px] w-full px-4">
                <>
                    {result && (
                        ...
                        >
                            ...
                            >
                                ...
                                <h2
                                    className={`text-3xl font-extrabold mb-2 drop-shadow-lg ${result.category === "BAD" ? "text-red-500" : "text-green-400"
                                        }`}
                                >
                                    ตกที่: {result.name}
                                </h2>
                    ...
                    <button
                        ...
                        disabled={isSpinning}
                        className={`relative group w-full max-w-xs py-4 rounded-xl font-black text-xl uppercase tracking-widest transition-all duration-100 ${isSpinning
                                ? "opacity-50 cursor-not-allowed translate-y-2"
                                : "hover:scale-105"
                            }`}
                    >
                        <div
                            className={`absolute inset-0 bg-red-900 rounded-xl transform translate-y-2 transition-transform duration-100 ${!isSpinning && "group-active:translate-y-1"
                                }`}
                        />
                        <div
                            className={`absolute inset-0 bg-gradient-to-b from-red-500 to-red-700 rounded-xl border-2 border-yellow-400/50 shadow-[inset_0_2px_10px_rgba(255,255,255,0.3),0_0_20px_rgba(239,68,68,0.6)] flex items-center justify-center transform transition-transform duration-100 ${!isSpinning && "group-active:translate-y-1"
                                }`}
                        />
                        <span
                            className={`relative text-white drop-shadow-md flex items-center gap-2 justify-center ${!isSpinning && "group-active:translate-y-1"
                                }`}
                        >
                            {isSpinning ? (
                                <>
