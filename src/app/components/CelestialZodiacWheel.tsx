...
const HOROSCOPE_POSITIONS: HoroscopeResult[] = [
  { sequence: 1, targetDegree: 180,  name: 'เจดีย์', category: 'GOOD', description: 'เป็นคนดี จะได้เป็นที่พึ่งพาแห่งตนและผู้อื่น (ปีชวด เริ่มต้น)' },
  { sequence: 2, targetDegree: 155,  name: 'ฉัตรเงิน', category: 'GOOD', description: 'จะมีลาภยศ มีความสุข เงินทองไหลมาเทมา' },
  { sequence: 3, targetDegree: 120,  name: 'คนคอขาด', category: 'BAD', description: 'เป็นคนอาภัพ หาที่พึ่งพาไม่ได้ ต้องระวังตัว' },
  { sequence: 4, targetDegree: 90,   name: 'เรือนหลวง', category: 'GOOD', description: 'จะประกอบด้วยยศถาบรรดาศักดิ์ เป็นเจ้าคนนายคน' },
  { sequence: 5, targetDegree: 65,   name: 'ปราสาท', category: 'GOOD', description: 'จะมีความสุข สบาย ก้าวหน้าในหน้าที่การงาน' },
  { sequence: 6, targetDegree: 30,   name: 'ราหู', category: 'BAD', description: 'น้ำใจไม่แน่นอน มักมีเรื่องเดือดร้อนใจ' },
  { sequence: 7, targetDegree: 10,   name: 'ฉัตรทอง', category: 'GOOD', description: 'ผู้ใหญ่จะอุปถัมภ์ค้ำชู มีเกียรติยศชื่อเสียง' },
  { sequence: 8, targetDegree: -15,  name: 'เทวดาขี่เต่า', category: 'GOOD', description: 'ชอบการเปลี่ยนแปลง แต่มักจะสุขสบายในบั้นปลาย' },
  { sequence: 9, targetDegree: -150, name: 'นาคราช', category: 'GOOD', description: 'มีอำนาจวาสนา บริวารเกรงใจ' },
  { sequence: 10, targetDegree: -120,name: 'แม่มด', category: 'GOOD', description: 'ผู้ใหญ่จะให้ความเมตตาปรานี มีวิชาความรู้' },
  { sequence: 11, targetDegree: -90, name: 'พ่อหมอ', category: 'GOOD', description: 'เจ้านายและขุนนางผู้ใหญ่จะเมตตา เลี้ยงดู' },
  { sequence: 12, targetDegree: -60, name: 'คนต้องขื่อคา', category: 'BAD', description: 'ชะตาไม่คงที่ มักดีตอนต้นและลำบากตอนปลาย หรือต้องโทษ' },
];

// คอมโพเนนต์หลักที่ใช้ดึง SearchParams
    ...
    <div className="flex flex-col items-center justify-center min-h-screen bg-[radial-gradient(ellipse_at_center,_var(--tw-gradient-stops))] from-purple-900 via-slate-900 to-black gap-6 overflow-hidden relative font-sans">
      ...
      <div className="z-10 text-center mt-8">
        ...
        <p className="text-slate-400 text-sm mt-2 font-mono opacity-80 uppercase tracking-widest">
           {gender === 'male' ? '♂ ชาย' : '♀ หญิง'} • พ.ศ. {yearBE} • {monthName}
        </p>
      </div>

      {/* --- Wheel Container (มุมมองเดิมเป๊ะๆ) --- */}
      <div className="relative w-72 h-72 md:w-[450px] md:h-[450px] z-10 my-[-20px] scale-90 md:scale-100">
      ...
      {/* --- Action Area / Result (ปุ่ม 3D เดิม) --- */}
      <div className="z-10 pb-8 flex flex-col items-center min-h-[140px] w-full px-4">
        {result ? (
            <motion.div 
                initial={{ opacity: 0, y: 20 }}
                animate={{ opacity: 1, y: 0 }}
                className="text-center bg-black/80 p-6 rounded-xl border border-yellow-500/50 backdrop-blur-md shadow-2xl max-w-md w-full"
            >
                <h2 className={`text-3xl font-bold mb-2 drop-shadow-lg ${result.category === 'BAD' ? 'text-red-500' : 'text-green-400'}`}>
                    ตกที่: {result.name}
                </h2>
                <div className="w-12 h-1 bg-yellow-500/50 mx-auto mb-3 rounded-full"></div>
                <p className="text-yellow-100/90 text-md font-light leading-relaxed mb-5">
                    {result.description}
                </p>
                <button 
                    onClick={() => window.location.href = '/'}
                    className="w-full py-3 font-bold text-white transition-all duration-200 bg-gradient-to-r from-red-600 to-red-800 rounded-lg hover:from-red-500 hover:to-red-700 shadow-lg"
                >
                    ↻ ทำนายใหม่
                </button>
            </motion.div>
            ...
            disabled={isSpinning}
            className={`
                relative group w-full max-w-xs py-4 rounded-xl font-black text-xl uppercase tracking-widest transition-all duration-100
                ${isSpinning ? 'opacity-50 cursor-not-allowed translate-y-2' : 'hover:scale-105'}
            `}
            >
            <div className={`absolute inset-0 bg-red-900 rounded-xl transform translate-y-2 transition-transform duration-100 ${!isSpinning && 'group-active:translate-y-1'}`}></div>
            <div className={`
                absolute inset-0 bg-gradient-to-b from-red-500 to-red-700 rounded-xl border-2 border-yellow-400/50 
            ...
            `}></div>
            <span className={`relative text-white drop-shadow-md flex items-center gap-2 justify-center ${!isSpinning && 'group-active:translate-y-1'}`}>
                {isSpinning ? (
                    <>
...
// Wrap ด้วย Suspense เพื่อให้ใช้ useSearchParams ได้ใน Next.js
